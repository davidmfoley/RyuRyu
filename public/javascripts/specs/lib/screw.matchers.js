		...
		contain :{
			match: function(expected, actual) {
				for (var i = 0; i < actual.length; i++) {
					if (matchers.equal.match(actual[i],expected))
						return true;
				}
				return false;
			},
			...
			failure_message: function(expected, actual, not) {
				if (not) {
					return "expected " + $.print(actual) + " to not contain " + $.print(expected) + ", but it did";
				} else {
					return "expected " + $.print(actual) + " to contain " + $.print(expected) + ", but it did not";
				}
			}
		},
